...
import ImgProfiles from '../../../assets/img/default.jpg';
...
const ListContacts : React.FC<Props> = ({ setContact, valueSearch, chatList, setChatList }) => {
    ...
    const formatDate = (date: number)=> {
        ...
        if (dateFormat.getFullYear() === dateNow.getFullYear() && dateFormat.getMonth() === dateNow.getMonth() && dateFormat.getDate() === dateNow.getDate()) {
            return (
            ...
            if (dateOntem.getFullYear() === dateFormat.getFullYear() && dateOntem.getMonth() === dateFormat.getMonth() && dateOntem.getDate() === dateFormat.getDate()) {
                return "Ontem";
            }else{
                    ...
                    data?.map(item => (
                        <li onClick={() => onClickChat(item)} className="item" key={item.info_chat.id}>
                            <div className="areaImgProfile">
                                <img className="imgProfileChatList" src={ImgProfiles} alt={"Imagem de perfil do " + item.contact?.name } />
                            </div>
                            ...
                            <div className="areaInfoChatList">
                                ...
                                <div className="bottonItem">
                                    ...
                                    <div className="lastMessage">
                                        <span>
                                            {item.last_message?.send === 1 ? <MdCheck className="iconCheck" /> : <></>}
                                        </span>
...

                                    {
                                        item.info_chat.not_read > 0 ? 
                                            <div className="msgNotRead">
                                                {item.info_chat.not_read}
                                            </div>
                                        :
                                            <div></div>
                                    }
